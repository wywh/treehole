...
def onMessage(msg):
	...
	if not result and msg['text'] == '/accept':
		...
		return
	if msg['text'] == '/license' or msg['text'] == '/start':
		bot.sendMessage(chat_id,'Bot license agreement:\n'+
			'We guarantee that we will send a message in a completely anonymous way.The bot will only record your '+
			...
			'Bot code in github : https://github.com/Too-Naive/treeholn'+
			'Author : @stdssr')
		return
	if not result:
	...
	if result[0][2] == 1:
		...
		sql.close()
		return
	elif datetimeFromNow(result[0][5]) > MESSAGE_COOL_DOWN_CYCLE:
		...
		bot.sendMessage(chat_id,'Portal running hot! Unsafe to send messages. Estimated time to cooldown: %d seconds'%calctime(result[0][5]+datetime.timedelta(seconds=MESSAGE_COOL_DOWN_CYCLE),nowtimestamp))
		sql.close()
...
def calctime(timestamp,nowatime):
	global sec
	dec = datetimeFromNowEx(timestamp,nowatime)
	for x in sec:
		if dec <= x:
			return x
	return dec
